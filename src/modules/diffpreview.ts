...
const insertDiff = (navpop: Navpopup) => {
    ...
    const maxLines = +(pg.option.popupDiffMaxLines as string | number);
    if (oldlines.length > maxLines || newlines.length > maxLines) {
        truncated = true;
        inner = stripOuterCommonLines(oldlines.slice(0, maxLines), newlines.slice(0, maxLines), +(pg.option.popupDiffContextLines as string | number));
        oldlines = inner.a;
        newlines = inner.b;
    }
