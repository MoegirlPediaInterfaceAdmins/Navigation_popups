...
import { isString, simplePrintf } from "./tools.ts";
    ...
    export const setPopupTrailer = (str, id) => setPopupHTML(str, "popupData", id);
    export const fillEmptySpans = (args) => {
        let redir = true;
        let rcid;
    ...
    export const imageHTML = (article, idNumber) => simplePrintf('<a id="popupImageLink$1"><img align="right" valign="top" id="popupImg$1" style="display: none;"></img></a>', [idNumber]);
    export const popTipsSoonFn = (id, _when, popData) => {
        let when = _when;
        if (!when) {
            when = 250;
        }
    ...
    export const setPopupTipsAndHTML = (html, divname, idnumber, popData) => {
